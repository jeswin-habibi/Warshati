...
export default function InventoryPage() {
  ...
  for (const i of list) {
    const k = i.category?.trim() || t('inventory.uncategorized')
    const arr = groupsMap.get(k)
    if (arr) arr.push(i)
    else groupsMap.set(k, [i])
  }
  const grouped = [...groupsMap.entries()].sort((a, b) => a[0].localeCompare(b[0]))
      ...
      {isLoading ? (
        ...
        <div className="space-y-5">
          {grouped.map(([cat, rows]) => (
            <div key={cat}>
              <h2 className="mb-2 flex items-center gap-2 px-1 text-xs font-bold uppercase tracking-wide text-muted-foreground">
                {cat}
                <span className="rounded-full bg-secondary px-1.5 py-0.5 text-[10px]">{formatNumber(rows.length)}</span>
              </h2>
              <ul className="space-y-2">
                {rows.map((i) => (
                  <li key={i.id}><Row i={i} /></li>
                ))}
              </ul>
            </div>
          ))}
